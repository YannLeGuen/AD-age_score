################################################################################
## Simulate power for AD phenotype score versus case-control association tests ##
## *Simulation based on varying prevalence increases across the ages 60-100y ###
################################################################################
##################################################################### 07/02/2020
################################################################################
## Libraries and Input
...
# 
# # ml load R
# # Rscript statistical_power_simulation_Fig1_S1.R ${SLURM_ARRAY_TASK_ID}
...
df1 <- data.frame(Doubles=double(),Doubles=double(),Doubles=double(),Doubles=double(),
                 Doubles=double())
colnames(df1) <- c("OR","MAF","N", 'ageDiff',"PREV100"); df2 <- df1; df3 <- df1

ageDiffs <- c(0, 5, 10)
Ns <- c(1000, 5000)
# The prevalence at 100 is used to approximate a constant ORage between 60 and 100
PREV100s <- c(0.02,0.03,0.05,0.07,0.10,
    ...
    for (id3 in 1:length(Ns)) {
      for (id4 in 1:length(ageDiffs)) {
        for (id5 in 1:length(PREV100s)) {
          df1[c,] <- c(ORs[id1], MAFs[id2], Ns[id3], ageDiffs[id4], PREV100s[id5])
          c <- c + 1
        }
      }
    }
...
for (id1 in 1:length(ORs)) {
  for (id2 in 1:length(MAFs)) {
    for (id3 in 1:length(Ns)) {
      for (id4 in 1:length(ageDiffs)) {
        for (id5 in 1:length(PREV100s)) {
          df2[c,] <- c(ORs[id1], MAFs[id2], Ns[id3], ageDiffs[id4], PREV100s[id5])
          c <- c + 1
        }
      }
    }
...
for (id1 in 1:length(ORs)) {
  for (id2 in 1:length(MAFs)) {
    for (id3 in 1:length(Ns)) {
      for (id4 in 1:length(ageDiffs)) {
        for (id5 in 1:length(PREV100s)) {
          df3[c,] <- c(ORs[id1], MAFs[id2], Ns[id3], ageDiffs[id4], PREV100s[id5])
          c <- c + 1
        }
      }
    }
...
for (i in 1:10){
  ...
  ##############################################################################
  # Second control draw using auomatically generated age_mod term
  ...
  ##############################################################################
  # Second case draw using auomatically generated age_mod term
